...
/*
 * A QR code sits on screen while a room finds its laptops, which takes longer
 * than picking a Chromecast. Still single-use and still short-lived — the
 * point of a ticket is that nothing reusable ends up in a URL.
 */
const QR_TICKET_TTL_MS = 20 * 60 * 1000;
...
/*
 * Said when a handed-off show is still running but nothing can supply a photo.
 *
 * It has to be precise about the way out. Re-sharing does not revive a code —
 * it mints a new one — so telling somebody to ask the presenter to "open it
 * again" would send them back to a code that will never work. What does revive
 * a show is a screen that already holds a copy coming back online.
 */
const NO_SOURCE_MESSAGE = 'Nobody with a copy of this slideshow is online right now. '
  + 'It starts again as soon as a screen that has one comes back — otherwise ask the '
  + 'presenter to share it again, which gives a new code.';
...
class Broadcast {
  /*
   * `store` is optional and injected rather than imported. Without it this is
   * the pure in-memory relay it has always been — which is how the tests run
   * with no database — and with it a show outlives the process. Nothing below
   * waits on a write: durability is a nicety, and a broadcast should not fail
   * because Postgres hiccuped.
   */
  constructor({ secret, store = null }) {
    this.secret = secret;
    this.store = store;
  ...
  /**
   * Rebuild the shows that outlived the process.
   *
   * Only the identity and the clock come back. Everything to do with who is
   * watching, what is parked and which bytes were cached is deliberately left
   * empty: those describe connections that died with the old process, and every
   * one of them repairs itself within a poll. A viewer re-polls, a presenter
   * re-registers, a screen re-reports what it holds.
   *
   * A handed-off show restores exactly, because its screens derive the current
   * slide from createdAt, interval and the wall clock rather than from anything
   * the server remembers. A live show restores as "waiting for the presenter",
   * which is the truth: the tab that was driving it is gone until it comes back.
   */
  async restore() {
    ...
    for (const record of records) {
      if (this.sessions.has(record.code) || record.expiresAt <= now) continue;
      this.sessions.set(record.code, {
        ...record,
        /*
         * Presented as last seen now rather than long ago. A live show whose
         * host is genuinely gone is ended by the sweeper one host-timeout later,
         * which gives a presenter reconnecting after a deploy the same grace as
         * one whose wifi dropped, instead of killing the show on the first tick.
         */
        hostSeenAt: now,
        gen: 0,
  ...
  extend(session, ttlMs) {
    ...
    if (this.store) this.store.touch(session.code, session.expiresAt);
    this.touchHost(session);
  ...
  /* ── Lifecycle ──────────────────────────────────────────────────────────── */
    ...
    if (this.store) this.store.remove(code);
...
module.exports = {
  Broadcast, DEFAULT_TTL_MS, HOST_TIMEOUT_MS, TICKET_TTL_MS, QR_TICKET_TTL_MS, CACHE_IDLE_MS,
